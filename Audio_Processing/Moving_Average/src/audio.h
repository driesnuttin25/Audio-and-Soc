...
volatile int16_t delayBuffer[DELAY_BUF_SIZE]; // Delay buffer
volatile unsigned int delayIndex = 0; // Current index in delay buffer
...
void audio_test(void)
{
    ...
    while (1)
    {
        ...
        // Reading stereo data, consider only one channel for delay effect
        u32DataL = Xil_In32(I2S_DATA_RX_L_REG);
		u32DataR = Xil_In32(I2S_DATA_RX_R_REG);

        // Assuming 16-bit audio data, extract left channel data
        currentSample = (int16_t)(u32DataL & 0xFFFF);

        // Retrieve the delayed sample and update the current sample in the delay buffer
        delayedSample = delayBuffer[delayIndex];
        delayBuffer[delayIndex] = currentSample;

        int16_t processedSample = delayedSample;

        // Update delay buffer index
        delayIndex = (delayIndex + 1) % DELAY_BUF_SIZE;

        // Write the unprocessed current sample to the right channel
		Xil_Out32(I2S_DATA_TX_R_REG, u32DataR);
        // Write the processed (mixed) sample to the left channel
        Xil_Out32(I2S_DATA_TX_L_REG, processedSample);
    }
}
#endif
